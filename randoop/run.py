...
* 缺少依赖的工具（Randoop、Jacoco、Junit/Hamcrest）自动下载到 `baseline/randoop/lib`。
...
BUG_ID = 1
VERSION = "f"  # for d4j mode
BASE_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = BASE_DIR / "project"
LIB_DIR = BASE_DIR / "lib"

# HumanEval-Java default paths
...
# Stable coordinates (artifactId, version, groupId)
STABLE_COORDS = {
...
EXCLUDE_CLASSES = {
    # Lang: triggers classpath resolution issues with Randoop 3.1.x
...
def process_humaneval(humaneval_root: Path, time_limit: int, seed: int, class_limit: Optional[int],
    ...
    # ensure compiled classes (buggy versions are under humaneval.buggy)
