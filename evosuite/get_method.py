...
baseline/evosuite/Sampled_Dataset_For_All_Projects.csv.
...
def project_dir_name_from_coords(project: str) -> str:
	if project not in new_run.STABLE_COORDS:
		return project
	artifact, _, group = new_run.STABLE_COORDS[project]
...
def main():
	...
	parser.add_argument("--project", default=None, choices=new_run.DEFAULT_PROJECTS,
	...
	parser.add_argument("--out", default=None, help="输出 CSV 路径（默认 method_list/<name>_cc<k>.csv）")
	...
	if args.project:
		_, src_dir, _ = new_run.prepare_stable_project(args.project)
		root = Path(src_dir)
	...
	out_path = Path(args.out) if args.out else (BASE_DIR / "method_list" / default_name)
