...
def artifact_filename(artifact: str, version: str, classifier: Optional[str] = None) -> str:
    return f"{artifact}-{version}-{classifier}.jar" if classifier else f"{artifact}-{version}.jar"


def local_artifact_path(artifact: str, version: str, classifier: Optional[str] = None) -> Path:
    return LIB_DIR / artifact_filename(artifact, version, classifier)


def download_artifact(group: str, artifact: str, version: str, classifier: Optional[str] = None) -> Path:
    LIB_DIR.mkdir(exist_ok=True)
    dest = local_artifact_path(artifact, version, classifier)
    if dest.exists():
        return dest
    last_error = None
...
def run_evosuite_for_class(workdir: Path, evosuite_jar: Path, cp: str, classes_dir: Path,
                           target_class: str, time_limit: Optional[int], seed: Optional[int], criteria: str,
    ...
    if time_limit is not None and time_limit > 0:
        cmd.append(f"-Dsearch_budget={time_limit}")
        cmd.append(f"-Dglobal_timeout={time_limit}")
    if seed is not None:
...
def main():
    ...
    if (args.target_method or args.target_method_signature) and (test_count < args.min_tests):
        ...
        if tests_dir.exists():
            shutil.rmtree(tests_dir)
        retry_time_limit = (args.time_limit * args.min_tests_retry_mult) if (args.time_limit is not None and args.time_limit > 0) else args.time_limit
        run_evosuite_for_class(
